...
struct StructItem : public Item {
	...
	StructItem(Struct& s) : s(s), items(createItems()) {
	}
	...
	Item & add() override { throw std::runtime_error("not a sequence"); }
	void set(variant) override { throw std::runtime_error("not a value"); }
};
...
struct SequenceItem : public Item {
	...
	SequenceItem(Sequence& s) : s(s) {}
	Item & add(const char* name) override { throw std::runtime_error("not a structure"); }
	Item & add() override {
...
struct TupleItem : public Item {
	...
	TupleItem(std::tuple<Args...>& t) : t(t) {
